...
FetchBrain automatically batches individual recall requests into bulk API calls to reduce connections and improve performance for high-concurrency scrapers.

## How It Works

When running multiple concurrent requests, the SDK:

1. Queues individual recall requests
...
Batched into items:   [{ ref, request }, ...]
                           ↓
Single API call:      POST /v1/recall { items: [...] }
                           ↓
...
The `/v1/recall` API endpoint accepts multiple items in a single request:

- Maximum **100 items** per request
- Returns `known` and `unknown` arrays, each keyed by your `ref`
- Billable per known item
